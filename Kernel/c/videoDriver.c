...
void printChar(Point topLeft, char c, uint32_t color, char *font_name, unsigned int font_size){
	...
	for(int i = 0; i < desc->height * font_size; i++){
		for(int j = 0; j < desc->width * font_size; j++){
			int condition = desc->data[c * 16 + i / font_size];
...
void printString(Point topLeft, char *string, uint32_t color, char *font_name, unsigned int font_size){
	int i = 0;
	while(string[i] != 0){
		printChar(topLeft, string[i], color, font_name);
		topLeft.x += CHAR_WIDTH * font_size;
		i++;
	}
}

// ACA EMPIEZA LO COPAIDO
...
void vdPrint(const char c, Color fnt, Color bgd){
    switch (c) {
        case '\n':
            videoDriver_newline();
        break;
...
//dibujo un cuadrado
void vdFillRect (int x, int y, int pixelWidth, int pixelHeight, Color color){
    Color * pixel;
