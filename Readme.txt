...
Sobre ese esqueleto se desarrolla una **mini shell** que corre en **modo usuario** y ofrece utilidades básicas de depuración y demostración, junto con un pequeño juego interactivo.
...
- `inforeg` → Vuelca los registros de propósito general  
  *(requiere activar la captura con `CTRL`)*.
- `zerodiv` / `invopcode` → Fuerzan excepciones de división por cero y op-code inválido para probar el manejo de errores del kernel.
- `changeFontSize` → Permite ajustar el tamaño de fuente en tiempo de ejecución.
...
- [QEMU](https://www.qemu.org/) instalado localmente  
  Se utiliza el binario: `qemu-system-x86_64`.
